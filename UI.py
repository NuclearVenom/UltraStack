"""
UltraStack GUI  v3  —  PyQt5 front-end for ultrastack.py
Place in the same folder as ultrastack.py and run with:
    python ultrastack_gui.py

Requirements:
    pip install PyQt5 opencv-contrib-python numpy torch tqdm
"""
...
from PyQt5.QtWidgets import (
    ...
    QFrame, QSizePolicy, QToolTip, QMessageBox, QAction
)
from PyQt5.QtCore import Qt, QThread, pyqtSignal, QTimer, QSize, QRectF, QRect
from PyQt5.QtGui import (
    QColor, QFont, QPainter, QBrush, QPen,
    QLinearGradient, QDragEnterEvent, QDropEvent, QTextCursor,
    QPainterPath
)
...
def labeled_row(label_text: str, widget, tip_text: str = "",
    ...
    if tip_text:
        tip(widget, tip_text); tip(lbl, tip_text)
    return row
...
# — custom-painted, no stylesheet thrashing, no border pulse, no margin changes
# ══════════════════════════════════════════════════════════════════════════════

class WaveRunButton(QPushButton):
    """
    Fully custom-painted QPushButton.
    A soft highlight band sweeps left→right across the gradient every ~4 s.
    Absolutely no border animation, no setStyleSheet() calls in the timer —
    only canvas painting.  Hover and press change the gradient tones smoothly.
    """
    ...
    def _tick(self):
        self._wave = (self._wave + 0.005) % 1.0   # full sweep every ~4 s
        self.update()
    ...
    def paintEvent(self, _):
        ...
        # Base gradient colour based on state
        if self._down:
        ...
        # Shimmer band — soft gaussian-ish highlight sweeping left→right
...
class WorkerThread(QThread):
    ...
    def run(self):
        ...
        if cfg.get("lite"):
            self.log_line.emit("  ⚡  QUICK STACK mode active")
        self.log_line.emit("═" * 58)
    ...
    # ── Quick Stack pipeline ─────────────────────────────────────────────────
    def _run_lite(self, cfg, input_path: Path, ext: str, master_dark):
        """
        Minimal pipeline: load → optional ORB align → fast average → save.
        No SIFT grouping, no denoising/sharpening, no sigma clipping.
        Optimised for CPU — all operations use simple NumPy incremental mean.
        """
        import numpy as np
        import cv2
        ...
        self.log_line.emit(
            f"  Align  : {'none (skipped)' if skip_align else 'ORB'}"
            f"  |  Mode: average  |  Enhance: off"
        )
        ...
        if input_path.is_dir():
            ...
            images = []
            skip   = cfg.get("skip", 1)
            maxf   = cfg.get("max_frames") or None
            fi     = 0
            while True:
                ret, frm = cap.read()
                if not ret: break
                if fi % skip == 0:
                    images.append(frm)
                    if len(images) % 100 == 0:
                        self.log_line.emit(f"  Read {len(images)} frames…")
                fi += 1
        ...
        # Dark subtract if provided
        if master_dark is not None:
            self.log_line.emit("  Subtracting dark frame…")
            images = [us.subtract_dark(im, master_dark) for im in images]

        # Align
        if not skip_align and len(images) > 1:
            self.log_line.emit(f"  Aligning {len(images)} frames with ORB…")
            base_gray = cv2.cvtColor(images[0], cv2.COLOR_BGR2GRAY)
            aligned   = [images[0]]
            for i, img in enumerate(images[1:], 1):
                aligned.append(us.align_frame(base_gray, img, "orb"))
                if i % 50 == 0:
                    self.log_line.emit(f"  Aligned {i}/{len(images)-1}…")
            images = aligned

        # Fast incremental average (stable, O(1) memory per pixel)
        self.log_line.emit(f"  Stacking {len(images)} frames (fast average)…")
        acc = images[0].astype("float64")
        for k, img in enumerate(images[1:], 1):
            acc += (img.astype("float64") - acc) / (k + 1)
            if k % 100 == 0:
                self.log_line.emit(f"  Stacked {k+1}/{len(images)}…")

        result = np.clip(acc, 0, 255).astype("uint8")

        # Adjust output extension to match chosen lite format
...
class GPUProbeThread(QThread):
    ...
    # text, kind("ok"/"warn"/"err"), device, torch_module
    ...
    def run(self):
        try:
            ...
            if device == "cuda":
                ...
                self.result.emit(f"✓  GPU: {name}  ({vram:.1f} GB VRAM)",
                                 "ok", device, torch_mod)
            else:
                self.result.emit("⚠  No CUDA GPU — CPU mode", "warn", device, torch_mod)
        except Exception as e:
            self.result.emit(f"✗  GPU probe failed: {e}", "err", "cpu", None)
...
class UltraStackGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        # GPU probe runs in background — UI is already interactive
        self._gpu_probe = GPUProbeThread()
        self._gpu_probe.result.connect(self._on_gpu_result)
        self._gpu_probe.start()

        self.statusBar().showMessage(
            "  ⏳  Probing GPU in background — UI is fully interactive."
        )
    ...
    def _on_gpu_result(self, text, kind, device, torch_mod):
        ...
        self._gpu_badge.setText(text)
        self._gpu_badge.setStyleSheet(
            f"color:{col}; font-size:13px; font-weight:600; background:transparent;"
        )
        self.statusBar().showMessage(f"  {text}")

        # Route everything to the system log — no banner over the UI
        ...
        if kind == "err":
            self._sys_append(
                ...
                "    Fix: visit pytorch.org → install the correct build.\n"
    ...
    def _build_menu(self):
        ...
        hm = mb.addMenu("Help")
        a3 = QAction("About", self);      a3.triggered.connect(self._show_about);  hm.addAction(a3)

    # ── Header ────────────────────────────────────────────────────────────────
    def _build_header(self) -> QWidget:
        hdr = GradientHeader()
        lay = QHBoxLayout(hdr); lay.setContentsMargins(28, 0, 28, 0)

        title = QLabel("ULTRASTACK"); title.setObjectName("title")
        sub   = QLabel("GPU-Accelerated Astronomical Image Stacking"); sub.setObjectName("subtitle")
        lv = QVBoxLayout(); lv.setSpacing(3)
        lv.addWidget(title); lv.addWidget(sub)

        self._gpu_badge = QLabel("⏳  Probing GPU…")
        self._gpu_badge.setStyleSheet(f"color:{C['warning']}; font-size:13px; background:transparent;")
        self._gpu_badge.setAlignment(Qt.AlignRight | Qt.AlignVCenter)

        lay.addLayout(lv); lay.addStretch(); lay.addWidget(self._gpu_badge)
        return hdr
    ...
    def _build_io_group(self) -> QGroupBox:
        ...
        self._drop = DropZone(); self._drop.dropped.connect(self._on_drop)
        ...
        ri = QHBoxLayout(); ri.addWidget(QLabel("Input:")); ri.addWidget(self._inp, 1); ri.addWidget(bi)
        lay.addLayout(ri)
        ...
        bo = QPushButton("💾  Save as"); tip(bo, "Choose output path and format"); bo.clicked.connect(self._browse_output)
        ro = QHBoxLayout(); ro.addWidget(QLabel("Output:")); ro.addWidget(self._out, 1); ro.addWidget(bo)
        lay.addLayout(ro)

        self._dark = QLineEdit()
        self._dark.setPlaceholderText("Optional: folder of dark calibration frames…")
        tip(self._dark,
            "Dark frames captured at same ISO/gain, exposure, and temperature.\n"
            "Median-stacked into a master dark and subtracted from every light frame.\n"
            "Essential for reducing thermal noise in long exposures.")
        bd = QPushButton("🌑  Dark folder"); tip(bd, "Select dark calibration folder"); bd.clicked.connect(self._browse_dark)
        rd = QHBoxLayout(); rd.addWidget(QLabel("Darks:")); rd.addWidget(self._dark, 1); rd.addWidget(bd)
        lay.addLayout(rd)
    ...
    def _tab_stacking(self) -> QWidget:
        ...
        tip(self._mode,
            "Stacking algorithm:\n\n"
            "average  — Mean per pixel. Fast, GPU-accelerated. Best for high\n"
            "           frame-count planetary/video captures.\n\n"
            "median   — Median per pixel. Auto-removes hot pixels, satellites,\n"
            "           and cosmic rays. Memory-intensive for large stacks.\n\n"
            "sigma    — Sigma-clipping mean (iterative outlier rejection).\n"
            "           Professional standard: DSS, Siril, PixInsight.\n"
            "           Best SNR for deep-sky imaging.\n\n"
            "maximum  — Brightest pixel wins. Star trails, lightning, aurora.\n\n"
            "minimum  — Darkest pixel wins. Background subtraction / gradient removal.")
        ...
        tip(self._sig_lo, "Reject pixels below  mean − σ_low × std_dev\nTypical range: 1.5–3.0")
        self._sig_hi = QDoubleSpinBox(); self._sig_hi.setRange(0.5,10); self._sig_hi.setSingleStep(0.5); self._sig_hi.setValue(2.0)
        tip(self._sig_hi, "Reject pixels above  mean + σ_high × std_dev\nTypical range: 1.5–3.0")
        self._sig_it = QSpinBox(); self._sig_it.setRange(1,10); self._sig_it.setValue(3)
        tip(self._sig_it, "Sigma-clipping iterations.\n2–5 is typical. Diminishing returns above 5.")
        ...
        lay.addWidget(self._sigma_box)
        self._sigma_box.setVisible(False)

        self._stitch = QCheckBox("Stitch stacked groups into panorama")
        tip(self._stitch, "Stitch all groups into a single panorama after stacking.\nRequires ≥20–30% image overlap. Ignored for video/SER.")
        lay.addWidget(self._stitch)

        self._thresh = QSpinBox(); self._thresh.setRange(5,500); self._thresh.setValue(20)
        tip(self._thresh, "Min SIFT feature matches to group two images together.\nHigher = stricter. Raise to 50–100 for panorama tiles.")
    ...
    def _tab_alignment(self) -> QWidget:
        ...
        tip(self._align,
            "Alignment method before stacking:\n\n"
            "none  — No alignment. Use only when images are pre-aligned\n"
            "        (tracked mount, fixed camera).\n\n"
            "orb   — ORB feature matching + RANSAC homography. Fast, robust.\n"
            "        Ideal for planetary, landscape, and building captures.\n\n"
            "ecc   — Enhanced Correlation Coefficient (sub-pixel accuracy).\n"
            "        Best for star fields and deep-sky targets.\n"
        ...
        tip(self._orb_feat, "Max ORB keypoints per image.\n2000–5000: fast.  5000–10000: better for low-contrast images.")
        lay.addLayout(labeled_row("Max features:", self._orb_feat, label_width=130))

        info = QLabel("ℹ  ECC uses Euclidean motion (rotation + translation),\n"
                      "   200 iterations at 1e-7 precision.\n"
                      "   Falls back to ORB if convergence fails.")
    ...
    def _tab_video_ser(self) -> QWidget:
        ...
        tip(self._skip, "Use every Nth frame.\nskip=1 → all frames  |  skip=2 → 50%  |  skip=5 → 20%")
        ...
        tip(self._maxf, "Stop after N frames. 0 = all frames.\nPlanetary SER: typical best-frame count is 10–30% of total.")
        lay.addLayout(labeled_row("Max frames (0=all):", self._maxf, label_width=185))

        lay.addWidget(h_line())
        ser = QLabel(
            "SER is the standard capture format for:\n"
            "  🪐  Planetary  •  🌙  Lunar  •  ☀  Solar imaging\n\n"
            "UltraStack handles:\n"
            "  • 8-bit & 16-bit pixel depths\n"
            "  • All Bayer patterns (RGGB, GRBG, GBRG, BGGR)\n"
            "  • RGB / BGR / Mono colour modes\n"
            "  • Observer, telescope & instrument metadata\n\n"
            "Recommended for SER:\n"
    ...
    def _tab_postprocess(self) -> QWidget:
        ...
        tip(self._enhance,
            "Two post-processing steps on the final image:\n\n"
            "1. fastNlMeansDenoisingColored  (h=5, hColor=5)\n"
            "   Conservative noise removal — preserves fine detail.\n\n"
            "2. Unsharp mask via Laplacian kernel\n"
            "   Sharpens edges and fine structure.\n\n"
        ...
        tip(self._stretch,
            "Remap the 0.5th–99.5th percentile range to 0–255 per channel.\n"
            "Reveals faint detail in deep-sky and SER captures.\n"
            "Not needed for bright planetary surfaces or daytime shots.")
    ...
    def _tab_quickstack(self) -> QWidget:
        ...
        tip(self._lite,
            "Quick Stack overrides all Stacking / Alignment / Post-Process\n"
            "settings and runs the fastest possible pipeline:\n\n"
            "  • Average stacking (incremental, no GPU required)\n"
            "  • Optional ORB alignment (skips slow SIFT grouping)\n"
            "  • No denoising, sharpening, or histogram stretch\n"
            "  • No dark subtraction, no hot pixel removal\n"
            "  • No panorama stitching\n\n"
            "Ideal for: CPU-only machines, quick previews, burst photos,\n"
            "and large video/SER files where speed matters more than quality.")
        lay.addWidget(self._lite)
        lay.addWidget(h_line())

        card = QLabel(
            "⚡  Quick Stack  —  Maximum Speed, Minimum Resources\n\n"
            "Skips SIFT feature-grouping (the slowest step) and stacks\n"
            "directly with a fast incremental mean. ORB alignment is\n"
            "optional and runs in a single lightweight pass.\n\n"
            "Typical speed vs Full pipeline (20 × 6 MP images, CPU):\n\n"
            "   Full pipeline   ≈  40–60 s   (SIFT + align + enhance)\n"
            "   Quick Stack     ≈   3–6 s    (load + ORB + average)\n\n"
            "Use Quick Stack for a fast preview first, then run the\n"
            "full pipeline on your best frames for the final result."
        )
        card.setWordWrap(True)
        card.setStyleSheet(
            f"color:{C['text_dim']}; font-size:13px; line-height:155%;"
            f"background:{C['surface2']}; border:1px solid {C['border']};"
            f"border-radius:8px; padding:14px;"
        )
        lay.addWidget(card)
        lay.addWidget(section_label("Quick Stack Options"))

        self._lite_no_align = QCheckBox("Skip alignment entirely  (pure average — fastest possible)")
        tip(self._lite_no_align,
            "Skip even ORB alignment.\n"
            "Frames are stacked in raw sequential order — no motion correction.\n\n"
            "Use only when images are pre-aligned (tracked mount,\n"
            "fixed tripod, or previously registered frames).")
        lay.addWidget(self._lite_no_align)

        self._lite_fmt = QComboBox()
        self._lite_fmt.addItems(["jpg", "png", "tif"])
        tip(self._lite_fmt,
            "Output format for Quick Stack.\n"
            "jpg is fastest to write and smallest — ideal for previews.\n"
            "tif for 16-bit lossless output.")
    ...
    def _tab_advanced(self) -> QWidget:
        ...
        tip(self._nogpu, "Force CPU even if a GPU is available.\nUse when VRAM is insufficient or for reproducible results.")
        lay.addWidget(self._nogpu)

        lay.addWidget(h_line()); lay.addWidget(section_label("Memory / Batch Sizing"))
        mem = QLabel(
            "UltraStack auto-estimates a safe GPU batch size using 70% of\n"
            "available VRAM. For very large images or limited VRAM it uses\n"
            "incremental batch stacking with weighted mean merging.\n\n"
            "If you get CUDA out-of-memory errors:\n"
            "  1. Enable 'Force CPU' above\n"
            "  2. Use average or sigma mode (most memory-efficient)\n"
            "  3. Increase frame skip to reduce frame count"
        )
        ...
        dep = QLabel(
            ...
            "  pip install astropy\n\n"
            "For GPU — install torch with the correct CUDA build:\n"
            "  pytorch.org → Get Started → select your CUDA version"
        )
        dep.setStyleSheet(
    ...
    def _build_right(self) -> QWidget:
        ...
        self._console.setLineWrapMode(QTextEdit.WidgetWidth)
        ...
        # Appears only when GPU probe reports an issue
        ...
        self._sys_toggle = QPushButton("▲  Hide")
        self._sys_toggle.setFixedWidth(82)
        ...
        tip(self._sys_toggle,
            "Toggle the system diagnostics panel.\n"
        ...
        self._syslog_body = QTextEdit()
        self._syslog_body.setObjectName("syslog")
        self._syslog_body.setReadOnly(True)
        self._syslog_body.setFixedHeight(115)
        self._syslog_body.setLineWrapMode(QTextEdit.WidgetWidth)
        sl.addWidget(self._syslog_body)
        lay.addWidget(self._syslog_panel)
        return w
    ...
    def _build_bottom_bar(self) -> QWidget:
        ...
        self._stop_btn.setObjectName("danger"); self._stop_btn.setFixedSize(110, 46)
        self._stop_btn.setVisible(False)
        tip(self._stop_btn, "Abort the current processing job.\nPartial output may not be saved.")
        self._stop_btn.clicked.connect(self._stop_job); lay.addWidget(self._stop_btn)

        self._run_btn = WaveRunButton("▶   Run UltraStack")
        tip(self._run_btn,
            "Start the stacking pipeline with the current settings.\n\n"
            "Runs in a background thread — UI stays fully responsive.\n"
            "Progress is printed to the Processing Log on the right.\n\n"
            "Enable ⚡ Quick Stack tab for a fast CPU-optimised preview.")
        self._run_btn.clicked.connect(self._run_job); lay.addWidget(self._run_btn)
        return bar

    # ── Signal handlers ───────────────────────────────────────────────────────
    def _on_mode_changed(self, m: str):
        self._sigma_box.setVisible(m == "sigma")
    ...
    def _on_input_changed(self, text: str):
        ...
        p = Path(text)
        if p.is_dir():
            self._type_lbl.setText(f"📁  Folder  —  {p.name}")
        elif p.suffix.lower() == ".ser":
            self._type_lbl.setText(f"🔭  SER file  —  {p.name}")
        elif p.suffix.lower() in {".mp4",".avi",".mov",".mkv",".wmv",".webm"}:
            self._type_lbl.setText(f"🎬  Video  —  {p.name}")
        else:
            self._type_lbl.setText(f"📄  {p.name}")
    ...
    def _run_job(self):
        if not ULTRASTACK_AVAILABLE:
            QMessageBox.critical(self, "Import Error",
                ...
                "Ensure ultrastack.py is in the same folder as this script.")
            return

        inp = self._inp.text().strip()
        if not inp:
            QMessageBox.warning(self, "No input", "Please specify an input folder, video, or SER file.")
            return

        out = self._out.text().strip()
        if not out:
            ts = datetime.now().strftime("%m%d_%H%M%S")
            out = f"ultrastack_{ts}.tif"
            self._out.setText(out)

        max_f = self._maxf.value()
        lite  = self._lite.isChecked()

        cfg = dict(
            input          = inp,
            output         = out,
            dark           = self._dark.text().strip() or None,
            mode           = self._mode.currentText(),
            align          = self._align.currentText(),
            enhance        = self._enhance.isChecked(),
            stretch        = self._stretch.isChecked(),
            stitch         = self._stitch.isChecked(),
            hot_pixels     = self._hotpx.value(),
            skip           = self._skip.value(),
            max_frames     = max_f if max_f > 0 else None,
            threshold      = self._thresh.value(),
            sigma_low      = self._sig_lo.value(),
            sigma_high     = self._sig_hi.value(),
            sigma_iters    = self._sig_it.value(),
            lite           = lite,
            lite_skip_align= self._lite_no_align.isChecked(),
            lite_fmt       = self._lite_fmt.currentText().split()[0],
            _device        = self._device,
            _torch         = self._torch,
        )
        if self._nogpu.isChecked():
            cfg["_device"] = "cpu"; cfg["_torch"] = None

        self._console.clear()
        tag = "⚡ Quick Stack" if lite else "Full Pipeline"
        self._log(f"Starting job  [{tag}]  —  {datetime.now().strftime('%H:%M:%S')}")
    ...
    def _on_finished(self, ok: bool, msg: str):
        ...
        if ok:
            ...
            QMessageBox.information(self, "UltraStack — Complete",
                                    f"✓  Stacking complete!\n\nSaved to:\n{msg}")
        else:
            self._log(f"\n  ✗  Failed: {msg}")
    ...
    def _log(self, text: str):
        ...
        # Section banners: ── STEP N — Title ──
        m = re.search(r"─+\s*(STEP\s*\d+.*?)\s*─+", t)
        if m or ("STEP" in t and "──" in t):
            label = (m.group(1).strip() if m else t).replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")
            html = (
        ...
        elif t.startswith("✓") or "DONE" in t or "done" in t.lower():
    ...
    def _show_about(self):
        QMessageBox.about(self, "About UltraStack v2",
            "<b>UltraStack v2</b><br>"
            "GPU-Accelerated Astronomical Image Stacking<br><br>"
            "Stack image folders, video files, or SER captures<br>"
            "using Average / Median / Sigma / Max / Min stacking.<br><br>"
            "<b>⚡ Quick Stack</b> gives a fast CPU-optimised preview.<br><br>"
            "<b>Backend :</b> ultrastack.py<br>"
            "<b>GUI     :</b> PyQt5<br><br>"
            "Images  : JPG, PNG, TIFF, FITS, BMP, WEBP, EXR<br>"
            "Video   : MP4, AVI, MOV, MKV, WMV, WEBM, M4V, TS<br>"
            "Astro   : SER (all Bayer / Mono / RGB / BGR, 8 & 16-bit)")
